...
term.getcurpos = function()
	...
	--
	if true then
		local n = os.getenv('LINES')
		local m = os.getenv('COLUMNS')
		local unistd = require'posix'.unistd
		if n and m then
			return tonumber(n), tonumber(m)
		else
			local ttyname = string.sub(unistd.ttyname(), 1, 8)
			if ttyname == '/dev/con' then
				return 25, 80
			end
			if  ttyname == '/dev/pts' then
				return 30, 120
			end
...
term.savemode = function()
	...
	local mode, msg, e = T.tcgetattr(0)
	return mode or nil, e, msg
end

term.restoremode = function(mode)
	local ret, err = T.tcsetattr(0, 0, mode)
	return ret
end
...
local term = package.preload.plterm()
...
Edition
	...
	^W		forward search (plain text, case sensitive)
	^Y		yank (paste)
	esc-5		replace
...
function ple(filename)
	-- process argument
	editor.quit = false;
	local ll, fname 
	if filename then
		ll, err = readfile(filename) -- load file as a list of lines
